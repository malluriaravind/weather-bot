...
class Actionsavefirstname(Action):
    ...
    def name(self) -> Text:
        return "action_save_first_name"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        print(tracker.get_slot('first_name'))
        return [SlotSet('first_name',tracker.latest_message['text'])]


def datastore(first_name):
    conn=sqlite3.connect('cov.db')
    mycursor = conn.cursor()
    mycursor.execute("""CREATE TABLE IF NOT EXISTS my_info (Name TEXT);""")
    mycursor.execute("INSERT INTO my_info VALUES (?)",(first_name,))
    conn.commit()
...
class Weather(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        possible_cities=["Auckland","Wellington","Christchurch", "San Jose", "Sunnyvale","Georgia","Texas","New York","Florida","Arizona","Los Angeles","Michigan"]
